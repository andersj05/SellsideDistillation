...
class PipelineTests(unittest.TestCase):
    ...
    def test_end_to_end_case_matrix_and_comparison_parity(self):
        ...
        self.assertTrue(
            all(r["numeric_correct"] == 15 and r["questions_answered"] == 3 for r in clean)
        )
    ...
    def test_replay_no_adapter_or_network_and_integrity_detection(self):
        ...
        with (
            patch(
                "research_lab.runtime.FixtureAdapter.generate",
                side_effect=AssertionError("Replay generated new work"),
            ),
            patch.object(
                socket,
                "create_connection",
                side_effect=AssertionError("Replay made network request"),
            ),
        ):
            with redirect_stdout(io.StringIO()):
                self.assertEqual(
                    main(["--root", str(self.root), "replay", "--run", result["run_id"]]), 0
                )
