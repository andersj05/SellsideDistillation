...
def report_markdown(task: Task, claims: list[Claim]) -> str:
    blocks = ["# Synthetic earnings update",
    ...
    for section in task.required_sections:
        ...
        paragraphs = [f"{claim.text} [^{i}]" for i, claim in enumerate(claims, 1) if claim.report_section == section]
    ...
    blocks.extend(f"[^{i}]: Sources: {', '.join(claim.supporting_span_ids) or 'none'}. "
                  f"Calculations: {', '.join(claim.calculation_ids) or 'none'}." for i, claim in enumerate(claims, 1))
    return "\n\n".join(blocks) + "\n"
...
def markdown_html(text: str) -> str:
    ...
    for block in text.strip().split("\n\n"):
        if block == "## Source and calculation notes":
            ...
            safe = re.sub(r"\[\^(\d+)\]", lambda m: f"<a href='#claim-{m[1]}' aria-label='Inspect claim {m[1]}'>[{m[1]}]</a>", escape(block))
...
def status_tag(status: str) -> str:
    label = {"passed_automated_checks": "Automated checks passed", "completed": "Completed"}.get(status, status.replace("_", " ").capitalize())
    return f"<span class='tag {escape(status, quote=True)}'>{escape(label)}</span>"


def render_run(run_dir: Path, manifest: dict, task: Task, packet: dict, findings: dict, evaluation: dict, playbook: dict) -> Path:
    research = evaluation["scorecards"]["research"]
    issues = "".join(f"<div class='issue'>{status_tag(i['severity'])}<h3>{escape(i['code'].replace('_', ' '))}</h3><p>{escape(i['message'])}</p></div>" for i in evaluation["issues"])
    if not issues:
    ...
    for number, claim in enumerate(findings["claims"], 1):
        links = " · ".join(f"<a href='#{anchors[s]}'>{escape(documents[spans[s]['document_id']]['original_filename'])} / {escape(spans[s]['locator'])}</a>"
                           for s in claim["supporting_span_ids"] if s in spans)
        claims_html.append(f"<article class='claim' id='claim-{number}'><span class='eyebrow'>Claim {number} · {escape(claim['claim_type'])} · {escape(claim['origin'])}</span>"
    ...
    for calc in findings["calculations"]:
        for metric, value in calc["outputs"].items():
            rows.append(f"<tr><td>{escape(calc['scenario'])}</td><td>{escape(metric.replace('_',' '))}</td>"
                        f"<td class='number'>{escape(value)}</td><td>{escape(calc['output_units'][metric])}</td></tr>")
    source_html = "".join(f"<details id='{anchors[s['span_id']]}'><summary>{escape(documents[s['document_id']]['original_filename'])} · {escape(s['locator'])}</summary>"
                          f"<p class='small'>Available: {escape(documents[s['document_id']]['available_at'])} · Native CSV record; human review pending</p>"
                          f"<pre>{escape(s['text'])}</pre><p class='small'>Source hash: {escape(s['document_sha256'])}</p></details>" for s in packet["spans"])
    rule_html = "".join(f"<details><summary>{escape(r['name'])} · {escape(r['origin'])}</summary><p>{escape(r['confidence'])}</p>"
                        f"<p><b>Applies:</b> {escape(r['applies_when'])}</p><p><b>Exception:</b> {escape(r['does_not_apply_when'])}</p>"
                        f"<p><b>Checks:</b> {escape(', '.join(r['checks']))}</p><p><b>Source spans:</b> {escape(', '.join(r['supporting_report_span_ids']) or 'None; proposed generic rule')}</p></details>" for r in playbook["rules"])
    body = (f"<div class='eyebrow'>Run review · {escape(manifest['variant'])}</div><h1>A forecast, with its evidence.</h1>"
...
def render_comparison(experiment_dir: Path, results: list[dict], assignments: dict, protocol: dict) -> Path:
    ...
    for r in results:
        ...
        rows.append(f"<tr data-case='{escape(r['case'], quote=True)}'><td><a href='{escape(href, quote=True)}'>{escape(r['case'].replace('_', ' '))}</a></td>"
    ...
    options = "".join(f"<option value='{escape(c, quote=True)}'>{escape(c.replace('_',' '))}</option>" for c in protocol["cases"])
    mapping = " · ".join(f"{escape(label)} = {escape(variant)}" for variant, label in assignments.items())
    body = ("<div class='eyebrow'>Development comparison · Foundation milestone</div><h1>Make the research inspectable.</h1>"
